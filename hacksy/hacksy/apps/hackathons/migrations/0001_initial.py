...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'Team'
        db.create_table('hackathons_team', (
        ...
        db.send_create_signal('hackathons', ['Team'])

        # Adding M2M table for field users on 'Team'
        db.create_table('hackathons_team_users', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('team', models.ForeignKey(orm['hackathons.team'], null=False)),
            ('user', models.ForeignKey(orm['auth.user'], null=False))
        ))
        db.create_unique('hackathons_team_users', ['team_id', 'user_id'])

        # Adding model 'Hackathon'
        db.create_table('hackathons_hackathon', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('name', self.gf('django.db.models.fields.CharField')(max_length=100)),
            ('slug', self.gf('django_extensions.db.fields.AutoSlugField')(allow_duplicates=False, max_length=50, separator=u'-', blank=True, unique=True, populate_from='name', overwrite=False)),
        ...
        db.send_create_signal('hackathons', ['Hackathon'])

        # Adding M2M table for field admin on 'Hackathon'
        db.create_table('hackathons_hackathon_admin', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('hackathon', models.ForeignKey(orm['hackathons.hackathon'], null=False)),
            ('user', models.ForeignKey(orm['auth.user'], null=False))
        ))
        db.create_unique('hackathons_hackathon_admin', ['hackathon_id', 'user_id'])
        ...
        db.create_unique('hackathons_hackathon_judges', ['hackathon_id', 'user_id'])

        # Adding M2M table for field teams on 'Hackathon'
        db.create_table('hackathons_hackathon_teams', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('hackathon', models.ForeignKey(orm['hackathons.hackathon'], null=False)),
            ('team', models.ForeignKey(orm['hackathons.team'], null=False))
        ))
        db.create_unique('hackathons_hackathon_teams', ['hackathon_id', 'team_id'])

        # Adding model 'Prize'
        db.create_table('hackathons_prize', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('position', self.gf('django.db.models.fields.IntegerField')(default=-1)),
            ('hackathon', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['hackathons.Hackathon'])),
            ('team', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['hackathons.Team'])),
        ))
        db.send_create_signal('hackathons', ['Prize'])


    def backwards(self, orm):
        # Deleting model 'Team'
        db.delete_table('hackathons_team')

        # Removing M2M table for field users on 'Team'
        db.delete_table('hackathons_team_users')

        # Deleting model 'Hackathon'
        db.delete_table('hackathons_hackathon')

        # Removing M2M table for field admin on 'Hackathon'
        db.delete_table('hackathons_hackathon_admin')

        # Removing M2M table for field judges on 'Hackathon'
        db.delete_table('hackathons_hackathon_judges')

        # Removing M2M table for field teams on 'Hackathon'
        db.delete_table('hackathons_hackathon_teams')

        # Deleting model 'Prize'
        db.delete_table('hackathons_prize')
    ...
    models = {
        ...
        'hackathons.hackathon': {
            ...
            'judges': ('django.db.models.fields.related.ManyToManyField', [], {'symmetrical': 'False', 'related_name': "'judge_set'", 'blank': 'True', 'to': "orm['auth.User']"}),

...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Hack.user'
        db.delete_column('hacks_hack', 'user_id')

        # Adding M2M table for field users on 'Hack'
        db.create_table('hacks_hack_users', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('hack', models.ForeignKey(orm['hacks.hack'], null=False)),
            ('hackathonmember', models.ForeignKey(orm['hackathons.hackathonmember'], null=False))
        ))
        db.create_unique('hacks_hack_users', ['hack_id', 'hackathonmember_id'])


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Hack.user'
        raise RuntimeError("Cannot reverse this migration. 'Hack.user' and its values cannot be restored.")
        # Removing M2M table for field users on 'Hack'
        db.delete_table('hacks_hack_users')
